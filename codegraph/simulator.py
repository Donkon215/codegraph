...
def _check_layer_violations(
    ...
    for change in changes:
        ...
        for src_pat, tgt_pat in layer_rules:
            if (fnmatch.fnmatch(src_mod, src_pat) and
                    fnmatch.fnmatch(tgt_mod, tgt_pat)):
                result.safe = False
                result.violations.append(SimViolation(
...
def _build_allowed_edges(
    ...
    """Build set of allowed subsystem-to-subsystem edges."""
    allowed: Set[Tuple[str, str]] = set()
    for sub in system.get("subsystems", []):
        sub_name = sub["name"]
        # Self-edges always allowed
        allowed.add((sub_name, sub_name))
    # Inter-subsystem edges from the architecture definition
    for edge in system.get("edges", []):
        allowed.add((edge["source"], edge["target"]))
    return allowed
...
def _detect_improvements(
    ...
    if removed_cycles:
        result.improvements.append(
            f"Removed {len(removed_cycles)} cycle(s)"
        )

    # Check if coupling decreased
    if result.coupling_delta < -0.05:
        result.improvements.append(
            f"Coupling reduced by {abs(result.coupling_delta):.3f}"
        )
...
def compare_candidates(
    ...
    *,
    ...
    for cand in candidates:
        ...
        results.append(CandidateSimResult(
